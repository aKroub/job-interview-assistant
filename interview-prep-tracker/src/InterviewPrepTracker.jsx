...
const InterviewPrepTracker = () => {
  ...
  const loadData = () => {
    try {
      const companiesResult = localStorage.getItem('companies');
      const questionsResult = localStorage.getItem('seenQuestions');

      if (companiesResult) {
        setCompanies(JSON.parse(companiesResult));
      }
      if (questionsResult) {
        setSeenQuestions(new Set(JSON.parse(questionsResult)));
      }
    } catch (error) {
  ...
  const saveData = (newCompanies, newSeenQuestions) => {
    try {
      localStorage.setItem('companies', JSON.stringify(newCompanies));
      if (newSeenQuestions) {
        localStorage.setItem('seenQuestions', JSON.stringify([...newSeenQuestions]));
      }
    } catch (error) {
