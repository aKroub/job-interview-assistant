...
import { resolveCompanyLogoUrl, guessDomain } from '../../utils/companyLogoUtils.js';
import { CompanyLogo } from '../shared/CompanyLogo.js';
import type { InterviewTypeName, Suggestion } from '../../types';
...
export function SuggestionCard({ suggestion, onDismiss, onAccept }: SuggestionCardProps) {
  const { Icon, colour } = TYPE_CONFIG[suggestion.type as InterviewTypeName] ?? { Icon: Calendar, colour: 'text-gray-600' };
  const action = (suggestion.action as string) || 'add';
  const config = ACTION_CONFIG[action as keyof typeof ACTION_CONFIG] || ACTION_CONFIG.add;
  const isEmailOnly = suggestion.source === 'gmail';
  const useAmberTheme = isEmailOnly && action === 'add';

  const logoUrl = resolveCompanyLogoUrl({ name: suggestion.companyName ?? '', domain: guessDomain(suggestion.companyName ?? '') });
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <CompanyLogo logoUrl={logoUrl} companyName={suggestion.companyName ?? ''} size={16} />
