...
describe('AddInterviewModal — video call link', () => {
  ...
  it('includes videoCallUrl in submitted data (add mode)', async () => {
    const user = userEvent.setup();
    const { onAdd } = setup();
    await user.selectOptions(screen.getByLabelText(/company/i), 'c1');
    await user.selectOptions(screen.getByLabelText(/type/i), 'Video Interview');
    ...
    expect(interviewData.videoCallUrl).toBe('https://zoom.us/j/999');
  });

  it('includes videoCallUrl in submitted data (edit mode)', async () => {
    const user = userEvent.setup();
    const { onEdit } = setupEditMode({
      interview: makeEditInterview({ type: 'Video Interview', videoCallUrl: 'https://meet.google.com/aaa-bbbb-ccc' }),
    });
    expect(screen.getByLabelText(/video call link/i)).toHaveValue('https://meet.google.com/aaa-bbbb-ccc');
    await user.click(screen.getByRole('button', { name: /save changes/i }));
    expect(onEdit).toHaveBeenCalledTimes(1);
    const [, , updates] = onEdit.mock.calls[0];
    expect(updates.videoCallUrl).toBe('https://meet.google.com/aaa-bbbb-ccc');
  });

  it('pre-populates videoCallUrl from initialValues when type is Video Interview', () => {
    setup({ initialValues: { companyId: 'c1', type: 'Video Interview', date: '2025-09-01', time: '10:00', videoCallUrl: 'https://zoom.us/j/555' } });
    expect(screen.getByLabelText(/video call link/i)).toHaveValue('https://zoom.us/j/555');
  });

  it('clears videoCallUrl when switching type away from Video Interview', async () => {
    const user = userEvent.setup();
    const { onAdd } = setup();
    await user.selectOptions(screen.getByLabelText(/company/i), 'c1');
    await user.selectOptions(screen.getByLabelText(/type/i), 'Video Interview');
    ...
    expect(screen.queryByLabelText(/video call link/i)).not.toBeInTheDocument();
    // Fill required fields and submit — videoCallUrl should be empty
    await user.type(screen.getByLabelText(/date/i), '2025-09-01');
    await user.type(screen.getByLabelText(/time/i), '10:00');
    await user.click(screen.getByRole('button', { name: /schedule/i }));
    const [, interviewData] = onAdd.mock.calls[0];
    expect(interviewData.videoCallUrl).toBe('');
  });
