...
describe('H1 — Rapid save/cancel cycling does not corrupt draft state', () => {
  it('reopening panel after cancel resets draft to current prop value', async () => {
    ...
    renderCard({ videoCallUrl: 'https://zoom.us/j/original' });
  ...
  it('three open-type-cancel cycles leave no stale draft', async () => {
    const user = userEvent.setup();
    const { onUpdateInterview } = renderCard({ videoCallUrl: 'https://zoom.us/j/stable' });
  ...
  it('save immediately followed by reopen shows the saved value (not stale prop)', async () => {
    ...
    const interview = makeInterview({ videoCallUrl: 'https://zoom.us/j/old' });
    ...
    const updated = makeInterview({ videoCallUrl: 'https://zoom.us/j/new' });
...
describe('H2 — Concurrent panel opens across multiple cards', () => {
  function renderDayWithTwoCards(handlers = {}) {
    ...
    const interviews = [
      makeInterview({
        ...
        videoCallUrl: 'https://zoom.us/j/acme', time: '09:00',
      }),
      makeInterview({
        id: 'i2', companyId: 'c2', companyName: 'Beta Inc',
        videoCallUrl: 'https://zoom.us/j/beta', time: '11:00',
      }),
    ];
  ...
  it('saving on one card does not affect the other card', async () => {
    ...
    expect(onUpdateInterview).toHaveBeenCalledWith('c1', 'i1', { videoCallUrl: 'https://zoom.us/j/acme-updated' });
...
describe('H3 — XSS vectors never reach the DOM via inline panel', () => {
  ...
  it.each(XSS_VECTORS)('blocks XSS vector from being saved: %s', async (xssUrl) => {
    const user = userEvent.setup();
    const { onUpdateInterview } = renderCard({ videoCallUrl: '' });
  ...
  it('previously stored XSS URI is never rendered as clickable href', async () => {
    ...
    renderCard({ videoCallUrl: 'javascript:alert(1)' });
  ...
  it('valid URL after XSS attempt saves correctly', async () => {
    ...
    expect(onUpdateInterview).toHaveBeenCalledWith('c1', 'i1', { videoCallUrl: 'https://zoom.us/j/safe' });
  });
});
...
describe('H4 — Enter/Escape key handling edge cases', () => {
  it('Enter on empty input saves empty string (clears the link)', async () => {
    const user = userEvent.setup();
    const { onUpdateInterview } = renderCard({ videoCallUrl: 'https://zoom.us/j/existing' });
  ...
  it('Enter on valid URL saves and closes panel', async () => {
    ...
    expect(onUpdateInterview).toHaveBeenCalledWith('c1', 'i1', { videoCallUrl: 'https://zoom.us/j/enter-save' });
...
// Parent updates videoCallUrl prop (e.g. via edit modal). User reopens panel.
...
describe('H5 — linkDraft syncs with external prop changes between panel sessions', () => {
  it('draft reflects updated prop value when panel is reopened after prop change', async () => {
    ...
    const interview = makeInterview({ videoCallUrl: 'https://zoom.us/j/v1' });
    ...
    // Parent updates the videoCallUrl prop
    const updated = makeInterview({ videoCallUrl: 'https://zoom.us/j/v2' });
    rerender(<InterviewCard interview={updated} {...props} />);
  ...
  it('draft reflects prop cleared to empty when panel is reopened', async () => {
    ...
    const interview = makeInterview({ videoCallUrl: 'https://zoom.us/j/will-be-cleared' });
    ...
    // Parent clears the videoCallUrl
    const updated = makeInterview({ videoCallUrl: '' });
    rerender(<InterviewCard interview={updated} {...props} />);
  ...
  it('draft reflects prop changing from empty to populated', async () => {
    ...
    const interview = makeInterview({ videoCallUrl: '' });
    ...
    // Parent sets a videoCallUrl (e.g. from a suggestion)
    const updated = makeInterview({ videoCallUrl: 'https://meet.google.com/new-meeting' });
    rerender(<InterviewCard interview={updated} {...props} />);
  ...
  it('multiple prop changes between opens always show latest value', async () => {
    ...
    let interview = makeInterview({ videoCallUrl: urls[0] });
    const { rerender } = render(<InterviewCard interview={interview} {...props} />);

    for (let i = 1; i < urls.length; i++) {
      // Rerender with next URL (without opening panel)
      interview = makeInterview({ videoCallUrl: urls[i] });
