...
describe('H2 — Video link round-trip through add/edit/cancel flows', () => {
  it('passes videoCallUrl through onAdd when scheduling via modal', async () => {
  ...
  it('passes videoCallUrl through onEdit when saving changes', async () => {
    const user = userEvent.setup();
    const onEdit = vi.fn();
    const interview = makeInterview({ type: 'Video Interview', videoCallUrl: 'https://meet.google.com/abc-defg-hij' });
  ...
  it('preserves empty videoCallUrl when none provided in add mode', async () => {
    const user = userEvent.setup();
    const onAdd = vi.fn();
    renderModal({
      onAdd,
  ...
  it('clears videoCallUrl in edit mode when user empties the field', async () => {
    const user = userEvent.setup();
    const onEdit = vi.fn();
    const interview = makeInterview({ type: 'Video Interview', videoCallUrl: 'https://zoom.us/j/old' });
    renderModal({ interview, onEdit });
...
describe('H3 — InterviewCard video link toggle edge cases', () => {
  it('shows toggle button for a valid https URL', () => {
    renderCard({ videoCallUrl: 'https://zoom.us/j/123' });
  ...
  it('shows toggle button for http URL (non-TLS)', () => {
    renderCard({ videoCallUrl: 'http://internal.company.com/meeting/42' });
    expect(screen.getByRole('button', { name: /show.*video call link/i })).toBeInTheDocument();
  });

  it('renders "Add" toggle when videoCallUrl is undefined', () => {
    renderCard({ videoCallUrl: undefined });
  ...
  it('renders "Add" toggle when videoCallUrl is null', () => {
    renderCard({ videoCallUrl: null });
    expect(screen.queryByRole('button', { name: /show.*video call link/i })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: /add.*video call link/i })).toBeInTheDocument();
  });

  it('renders "Add" toggle when videoCallUrl is empty string', () => {
  ...
  it('renders "Add" toggle for data: URI (invalid, not rendered as link)', () => {
    renderCard({ videoCallUrl: 'data:text/html,<h1>XSS</h1>' });
    expect(screen.queryByRole('button', { name: /show.*video call link/i })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: /add.*video call link/i })).toBeInTheDocument();
  });

  it('renders "Add" toggle for ftp: URI (invalid, not rendered as link)', () => {
    renderCard({ videoCallUrl: 'ftp://files.example.com/meeting' });
    expect(screen.queryByRole('button', { name: /show.*video call link/i })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: /add.*video call link/i })).toBeInTheDocument();
  });
  ...
  it('does NOT render toggle for Phone Interview type even with valid URL', () => {
    renderCard({ type: 'Phone Interview', videoCallUrl: 'https://zoom.us/j/123' });
    expect(screen.queryByRole('button', { name: /show.*video call link/i })).not.toBeInTheDocument();
  });

  it('does NOT render toggle for In-Person Interview type even with valid URL', () => {
    renderCard({ type: 'In-Person Interview', videoCallUrl: 'https://zoom.us/j/123' });
    expect(screen.queryByRole('button', { name: /show.*video call link/i })).not.toBeInTheDocument();
  });
  ...
  it('handles very long URLs without crashing', async () => {
    ...
    renderCard({ videoCallUrl: longUrl });
    await user.click(screen.getByRole('button', { name: /show.*video call link/i }));
  ...
  it('opens revealed link in new tab with security attributes', async () => {
    const user = userEvent.setup();
    renderCard({ videoCallUrl: 'https://meet.google.com/abc-defg-hij' });
    await user.click(screen.getByRole('button', { name: /show.*video call link/i }));
    const link = screen.getByRole('link', { name: /join.*video call/i });
...
describe('H4 — AddInterviewModal sanitises dangerous video URIs', () => {
  ...
  it.each(dangerousUris)('strips dangerous URI in edit mode: %s', async (uri) => {
    const user = userEvent.setup();
    const onEdit = vi.fn();
    const interview = makeInterview({ type: 'Video Interview', videoCallUrl: 'https://zoom.us/j/safe' });
    renderModal({ interview, onEdit });
