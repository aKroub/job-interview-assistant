  ...
  describe('AddInterviewModal trims whitespace on submit', () => {
    ...
    it('trims whitespace in edit mode too', async () => {
      ...
      const interview = makeInterview({
        type: 'Video Interview',
        videoCallUrl: '  https://zoom.us/j/edit-test  ',
      });
...
// ===========================================================================
// Hypothesis 3: Type switching in AddInterviewModal leaks videoCallUrl
//
...
// non-empty videoCallUrl.
// ===========================================================================

describe('H3 — Type switching edge cases in AddInterviewModal', () => {
  it('clears videoCallUrl when switching from Video to Phone and back to Video', async () => {
    const user = userEvent.setup();
    const onAdd = vi.fn();
    renderModal({ onAdd });
  ...
  it('submitting as Phone Interview after entering a video link yields empty videoCallUrl', async () => {
    const user = userEvent.setup();
    const onAdd = vi.fn();
    renderModal({ onAdd });
  ...
  it('switching through all types clears videoCallUrl each time', async () => {
    const user = userEvent.setup();
    const onAdd = vi.fn();
    renderModal({ onAdd });
    ...
    expect(onAdd).toHaveBeenCalledWith('c1', expect.objectContaining({
      videoCallUrl: 'https://zoom.us/j/2',
    }));
  });

  it('edit mode: switching type away from Video clears videoCallUrl in submission', async () => {
    const user = userEvent.setup();
    const onEdit = vi.fn();
    const interview = makeInterview({
      type: 'Video Interview',
      videoCallUrl: 'https://zoom.us/j/existing',
    });
    renderModal({ interview, onEdit });
...
//
// If the interview prop changes (e.g. videoCallUrl removed via edit), the
...
describe('H4 — InterviewCard prop changes with stale toggle state', () => {
  it('hides link section when re-rendered with empty videoCallUrl after toggle was open', async () => {
    const user = userEvent.setup();
    const props = {
      onDeleteInterview: vi.fn(),
      onEdit: vi.fn(),
    ...
    const interview = makeInterview({ videoCallUrl: 'https://zoom.us/j/123' });
    ...
    // Re-render with videoCallUrl removed
    const updatedInterview = makeInterview({ videoCallUrl: '' });
    rerender(<InterviewCard interview={updatedInterview} {...props} />);

    // Join call link should be gone since videoCallUrl is empty
    expect(screen.queryByRole('link', { name: /join.*video call/i })).not.toBeInTheDocument();
  ...
  it('hides link section when type changes from Video to Phone via re-render', async () => {
    ...
    const updatedInterview = makeInterview({ type: 'Phone Interview', videoCallUrl: 'https://zoom.us/j/123' });
    rerender(<InterviewCard interview={updatedInterview} {...props} />);
  ...
  it('toggle returns to collapsed state when videoCallUrl changes to a new URL', async () => {
    const user = userEvent.setup();
    const props = {
      onDeleteInterview: vi.fn(),
      onEdit: vi.fn(),
    ...
    const interview = makeInterview({ videoCallUrl: 'https://zoom.us/j/original' });
    ...
    const updatedInterview = makeInterview({ videoCallUrl: 'https://zoom.us/j/updated' });
    rerender(<InterviewCard interview={updatedInterview} {...props} />);
